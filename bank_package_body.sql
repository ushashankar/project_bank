CREATE OR REPLACE PACKAGE BODY BANK_PAC IS
...
RETURN NUMBER IS
  CURSOR CUR1(P_CUR_CUST_ID IN BANK_CUST.CUST_ID%TYPE) 
  IS 
...
END PROC_ADD_CUSTOMER;
 PROCEDURE PRC_ADD_CUST_CONTACT
...
BEGIN
 ...
 WHEN OTHERS THEN
  NULL;
END PRC_ADD_CUST_CONTACT;
PROCEDURE PRC_DEL_CUST(P_BANK_CUST_ID BANK_CUST.CUST_ID%TYPE) IS
BEGIN
 DELETE BANK_CUST WHERE BANK_CUST.CUST_ID=P_BANK_CUST_ID;
 --COMMIT;
END PRC_DEL_CUST;

PROCEDURE PRC_DEL_CUST(P_CUST_FIRST_NAME BANK_CUST.CUST_FIRST_NAME%TYPE) IS
BEGIN
 DELETE BANK_CUST WHERE CUST_FIRST_NAME=INITCAP(P_CUST_FIRST_NAME);
 --COMMIT;
END PRC_DEL_CUST;

PROCEDURE PRC_ADD_SB_ACCOUNT (
...
EXCEPTION
  ...
  WHEN OTHERS THEN
  NULL;
END PRC_ADD_SB_ACCOUNT;
PROCEDURE PRC_SB_DEPOSIT (P_TRANS_ACC_NO BANK_TRANSACTION.TRANS_ACC_NO%TYPE,
...
PROCEDURE PRC_SB_WITHDRAW (P_TRANS_ACC_NO BANK_TRANSACTION.TRANS_ACC_NO%TYPE,
                          P_TRANS_DESC BANK_TRANSACTION.TRANS_DESC%TYPE,
                          P_TRANS_AMT BANK_TRANSACTION.TRANS_AMT%TYPE) IS
V_COUNT NUMBER;
